...
def create_dataloader(path, imgsz, batch_size, stride,  opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='',mask_dir='', ret_mask=False, phase='training',device=None):
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, imgsz, batch_size,
                                      ...
                                      prefix=prefix, mask_dir=mask_dir, ret_mask=ret_mask, phase=phase,device=device)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',mask_dir='', ret_mask=False,phase='training',device=None):
    ...
    def __getitem__(self, index):
        ...
        imgs_pred_r=int(self.color_name.split(',')[0])
        imgs_pred_g=int(self.color_name.split(',')[1])
        imgs_pred_b=int(self.color_name.split(',')[2])
        
        imgs_pred_imgsize = (640,640)
        imgs_pred = torch.ones((1, 3, imgs_pred_imgsize[0], imgs_pred_imgsize[1]),dtype=torch.float32)
        imgs_pred[0, 0, :, :] = imgs_pred_r
        imgs_pred[0, 1, :, :] = imgs_pred_g
        imgs_pred[0, 2, :, :] = imgs_pred_b
        imgs_pred = imgs_pred.to(self.device)
        #0-255 to 0.0-1.0
        ...
        color_ref_path=os.path.join("/data/zhoujw/test_NSR_4distance/differentColor",self.color_name,filename)
